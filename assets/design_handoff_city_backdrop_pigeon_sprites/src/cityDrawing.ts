...
// Radial-style glow circle drawn as discrete rings (pixel art).
function pixelGlow(
  ctx: CanvasRenderingContext2D,
  cx: number, cy: number,
  radius: number,
  ...
  ctx.fillStyle = outer;
  filledCircle(ctx, cx, cy, radius);
  ctx.globalAlpha = 0.35;
  filledCircle(ctx, cx, cy, Math.max(1, radius - 3));
  // Inner core
  ctx.globalAlpha = 0.7;
  ctx.fillStyle = inner;
  filledCircle(ctx, cx, cy, Math.max(1, radius - 6));
  ctx.globalAlpha = 1;
  // Bright center
  ctx.fillStyle = inner;
...
function drawApartmentBlock(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, p: CityPalette, rng: RNG) {
  ...
  drawWaterTower(ctx, x + 40, y + 40, p, rng);
  ...
  drawSatDish(ctx, x + 240, y + 40, p, rng);
...
function drawParkBlock(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, p: CityPalette, rng: RNG) {
  ...
  for (const [tx, ty] of treeSpots) drawTree(ctx, tx, ty, p, rng);
...
function drawIndustrialBlock(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, p: CityPalette, rng: RNG) {
  ...
  drawSmokestack(ctx, x + 110, y + 28, p, rng);
  drawSmokestack(ctx, x + 270, y + 28, p, rng);
...
function drawWaterTower(ctx: CanvasRenderingContext2D, x: number, y: number, p: CityPalette, rng: RNG) {
...
function drawSkylight(
  ctx: CanvasRenderingContext2D,
  x: number, y: number, w: number, h: number,
  p: CityPalette, kind: "warm" | "cool",
) {
  // Frame
  rect(ctx, x - 2, y - 2, w + 4, h + 4, p.metalDark);
  rect(ctx, x - 1, y - 1, w + 2, h + 2, p.metal);
  // Glass
  const glow = kind === "warm" ? p.windowGlowWarm : p.windowGlowCool;
  const glowDim = kind === "warm" ? p.windowGlowWarmDim : p.windowGlowCoolDim;
  rect(ctx, x, y, w, h, glowDim);
  // Hot inner with lighter glow center
  rect(ctx, x + 2, y + 2, w - 4, h - 4, glow);
  // Cross muntins
  rect(ctx, x + Math.floor(w / 2) - 1, y, 2, h, p.metalDark);
  rect(ctx, x, y + Math.floor(h / 2) - 1, w, 2, p.metalDark);
  // Small bright highlight squares
  rect(ctx, x + 3, y + 3, 4, 2, "#fff8");
  rect(ctx, x + w - 7, y + h - 5, 4, 2, "#fff4");
}
...
function drawSatDish(ctx: CanvasRenderingContext2D, x: number, y: number, p: CityPalette, rng: RNG) {
  // Base
...
function drawSmokestack(ctx: CanvasRenderingContext2D, x: number, y: number, p: CityPalette, rng: RNG) {
  const r = 7;
...
function drawTree(ctx: CanvasRenderingContext2D, x: number, y: number, p: CityPalette, rng: RNG) {
...
function drawSmallTree(ctx: CanvasRenderingContext2D, x: number, y: number, p: CityPalette) {
  ctx.fillStyle = p.treeShadow;
  filledCircle(ctx, x + 1, y + 2, 6);
  ctx.fillStyle = p.tree;
  filledCircle(ctx, x, y, 6);
  ctx.fillStyle = p.treeHi;
  filledCircle(ctx, x - 2, y - 2, 2);
}
